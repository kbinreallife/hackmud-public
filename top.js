function(context, args) {
	...
	if(!dat) {
		...
		for(let i = 0; i < dat.length; i++) {
			...
			if(!db_user) {
				changes.push(`new appearance: ${current_user.name} ${current_user.rank}`)
			} else {
				...
				if(Math.abs(current_user.balance - db_user.balance) > 100000000000) {
					changes.push(`${current_user.name}'s balance changed: ${l.to_gc_str(db_user.balance)}`)
				}

				// if a user's rank is different than what we have stored, let me know.
				if (current_user.rank !== db_user.rank) {
					changes.push(`user position change: ${db_user.name} ${db_user.rank} -> ${current_user.rank}`)
				}
			}
		}
		...
		for(let db_user of dat) {
			if(!top.find(user => user.name) == db_user.name) {
				changes.push(`Left users.top: ${db_user.name}`)
			}
		}
		...
		// create a final message for users.top
		const ret = changes.length > 0 ? changes.join('\n') : 'no changes detected on users.top'
		...
		return #ms.chats.tell({ to: "trust", msg: ret })
	}
}
